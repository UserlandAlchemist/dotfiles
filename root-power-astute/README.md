...
- `nas-inhibit.service` - Systemd service that blocks sleep while NAS is in
  use
...
- `nas-inhibit.sudoers` - Allows `alchemist` to control
  nas-inhibit.service without password
...
- `astute-idle-suspend.timer` - Triggers idle check every 3
  minutes
...
- `powertop.service` - One-shot service that applies powertop
  tunings on boot
...
- `blacklist-watchdog.conf` - Disables watchdog modules
  (iTCO_wdt, iTCO_vendor_support)
- `blacklist-gpu.conf` - Disables AMD GPU drivers (amdgpu,
  radeon) on headless server
...
2. Audacious SSH to `astute-nas` host with command
   `start`
3. SSH forced command runs: `/usr/local/libexec/astute-nas-inhibit.sh`
4. Script validates command, runs: `sudo systemctl start
   nas-inhibit.service`
5. Service asserts sleep inhibitor, preventing suspension
6. After `nas-close` on Audacious, service stops and inhibitor
   releases
...
2. `astute-idle-check.sh` runs checks:
   - `w -h` - Any interactive SSH or local console sessions
     (pts/ or tty)?
   - `systemd-inhibit --list --what=sleep` - Any sleep inhibitors?
   - `find /srv/nas -maxdepth 4 -mmin -10` - Recent NAS file
     activity?
...
3. If Astute is up: Trigger `systemctl start
   astute-idle-suspend.service` via SSH
4. Display mako notification with result:
   - "Going to Sleep" - System was idle, now suspending
   - "Staying Awake - [reason]" - System has active
     inhibitor/sessions/activity
...
- Low urgency (gray, 8s): Status updates
   ...
   - "Wake on LAN - Sending magic packet to wake
     Astute..."
...
- Dedicated key `id_ed25519_astute_nas` (no passphrase, for
  automation)
- Authorized_keys forced command: only allows `start` or
  `stop`
...
- User `alchemist` can only:
   ...
   - Read astute-idle-suspend.service logs via
     journalctl
...
- Manual cleanup: `ssh astute sudo systemctl stop
  nas-inhibit.service`
...
**Note:** Sudoers files cannot be stowed directly
(symlinks violate security requirements). The install.sh
script uses `install -o root -g root -m 0440` to copy the
...
# should be 0
ssh astute \
  'systemd-inhibit --list --mode=block | grep sleep | wc -l'
```
