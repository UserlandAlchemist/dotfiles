...
**Configuration:** ZFS RAID1 encrypted root, systemd-boot UKI, Sway desktop,
...
- **Disks:** 2× NVMe (nvme0n1, nvme1n1) with 512MB ESP on each drive
  (mirrored via efi-sync) and remaining space for ZFS mirror
...
Expected result: Both NVMe drives have 512MB ESP and remaining space for
ZFS.
...
Create encrypted ZFS mirror pool with optimized settings and mount the root
dataset at `/mnt` for bootstrapping.
...
1. Create the pool with an alternate root at `/mnt`:
   ...
   **Why:** `ashift=12` matches 4K sectors. `compression=lz4` reduces write
   amplification with negligible CPU overhead. `encryption=aes-256-gcm`
   provides AEAD encryption. `-R /mnt` mounts datasets under `/mnt` for the
   install workflow.
...
- Run this guide verbatim on `test-audacious` VM using current Debian Trixie
  ISO
...
**Why systemd-networkd:** Consistent with systemd-boot philosophy, avoids
NetworkManager complexity.
...
4. Create wired network configuration (replace `Name=` with actual interface
   from `ip link`):
   ...
   **Note:** This will be overridden by `root-network-audacious` during
   dotfiles deployment.

Expected result: DHCP networking configured for primary ethernet
interface.
...
Expected result: GPU, CPU, and NIC firmware available for hardware
initialization.
...
Set up initramfs-tools to generate Unified Kernel Images for ZFS encrypted
pool unlock.

**Why initramfs-tools:** Provides reliable ZFS encrypted pool unlocking with
multiple vdevs.
...
Expected result: systemd-boot installed and configured with generated UKI as
default.
...
2. Add to fstab (replace UUIDs with actual values):
   ...
   ```ini
   # /etc/fstab
   UUID=<uuid0> /boot/efi vfat umask=0077,shortname=winnt 0 1
   UUID=<uuid1> /boot/efi-backup vfat umask=0077,shortname=winnt 0 1
   ```
...
1. Set up Jellyfin repository:
   ...
   printf '%s%s\n' \
     "deb [arch=amd64 signed-by=/usr/share/keyrings/jellyfin.gpg] " \
     "https://repo.jellyfin.org/debian trixie main" \
...
2. Set up Prism Launcher repository:
   ...
   ```sh
   repo_url=https://prism-launcher-for-debian.github.io/repo
   printf '%s%s%s%s\n' \
     "deb [arch=amd64 signed-by=/usr/share/keyrings/" \
     "prismlauncher-archive-keyring.gpg] " \
     "$repo_url " \
     "trixie main" \
...
Expected result: Third-party repositories configured for Jellyfin and Prism
Launcher packages.
...
1. Install desktop packages:
   ...
   ```sh
   apt install -y sway swaybg swayidle swaylock waybar wofi mako-notifier \
     xwayland grim slurp wl-clipboard xdg-desktop-portal-wlr \
...
Expected result: Sway desktop environment installed with autologin
configured.
...
**Goal:** User packages to `$HOME`, system packages to `/`. Real directories
for secrets.
...
2. Create real directories for secrets:
   ...
   **Why:** SSH keys and Borg passphrase are local secrets never committed.
   PSD writes runtime state.
...
4. Deploy user packages:
   ...
   stow profile-common bash-audacious bin-audacious emacs-audacious \
     ...
     pipewire-audacious ardour-audacious ncmpcpp-audacious \
     picard-audacious
   ```
...
7. Verify user deployment:
   ...
   ```sh
   ls -l ~/.bashrc ~/.local/bin/idle-shutdown.sh \
     ~/.config/sway/config  # should be symlinks
...
8. Deploy system packages:
   ...
   **Rule of thumb:** stow anything under `~` (including `~/.config` and
   `~/.local`). Use install scripts for anything outside `~` (`/etc`,
   `/usr/local`, `/usr/lib/systemd`, etc.) to avoid `/home` mount timing
   issues. Exception: mimeapps.list should be copied, not stowed (desktop tools
   modify it).
...
9. Enable services:
   ...
   sudo systemctl enable --now borg-offsite-audacious.timer \
...
system journal has early boot entries.

### Package reference

- `profile-common` (user): Shell profile baseline (shared across hosts)
- `bash-audacious` (user): Bash config, prompt, aliases, astute helpers
  (nas-open/close, ssh-astute)
- `bin-audacious` (user): Scripts: idle-shutdown, audio switching,
  game-performance, astute-status
- `emacs-audacious` (user): Editor config and custom theme
- `theme-audacious` (user): GTK theme, fonts (Topaz), icons (Amiga cursors),
  wallpapers
- `sway-audacious` (user): Sway + Waybar + Foot + Wofi + Mako (consolidated
  Wayland ecosystem)
...
- `mimeapps-audacious` (user): Default applications (PDF->zathura,
  http->firefox) - copied, not stowed
...
- `root-journald-audacious` (system): Journald config for minimal Debian + ZFS
  (ForwardToSyslog override, /var timing)
- `root-power-audacious` (system): Powertop tuning, udev autosuspend rules,
  SATA power policy
- `root-efisync-audacious` (system): Dual ESP rsync (efi-sync.path watches
  /boot/efi/EFI/Linux/)
- `root-cachyos-audacious` (system): Kernel/sysctl/I/O scheduler tuning
  (CachyOS-derived gaming optimizations)
- `root-network-audacious` (system): systemd-networkd wired ethernet with APT
  proxy auto-detection
- `root-firewall-audacious` (system): nftables host firewall (default-deny
  inbound)
- `root-borg-audacious` (system): Borg timers for local (Astute) and offsite
  (BorgBase) backups
- `root-sudoers-audacious` (system): Passwordless sudo for srv-astute.mount
  control (NAS mounting)
- `root-proaudio-audacious` (system): Real-time audio kernel tuning (rtprio
  limits, threadirqs)
...
**Goal:** Audacious wakes Astute on-demand for storage, prevents Astute
suspension during NAS use.
...
1. Add NFS mount to fstab:
   ...
   **Why noauto:** Mount controlled by systemd (`srv-astute.mount`) triggered
   via `nas-open` function.

2. Generate dedicated SSH key for NAS control:

   ```sh
   ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519_astute_nas \
     -C "audacious-nas" -N ""
   ```
...
4. On Astute, add to `~/.ssh/authorized_keys`:
   ...
   ```sh
   nas_key='command="/usr/local/libexec/astute-nas-inhibit.sh",restrict'
   nas_key="$nas_key ssh-ed25519 AAAA...KEY... audacious-nas"
   printf '%s\n' "$nas_key" >> ~/.ssh/authorized_keys
   ```

   **Why forced command:** SSH key can only toggle sleep inhibitor, nothing
   else.
...
Expected result: NAS wake-on-demand working, inhibitor active during use,
...
2. Format and enable swap:
   ...
   printf '%s\n' "/dev/zvol/rpool/swap none swap defaults,pri=10 0 0" \
     >> /etc/fstab
   ```

   **Note:** With 32GB RAM + zram, disk swap is rarely needed. Swappiness set
   to 150 in `root-cachyos-audacious`.
...
4. After reboot, verify system:
   ...
   ```sh
   # both nvme devices online
   zpool status
   # UKI present and set as default
   bootctl list
   # should be empty
   systemctl --failed
   # borg timers scheduled
   systemctl list-timers
   ```

Expected result: System boots via systemd-boot UKI, prompts for ZFS passphrase,
autologins to Sway.
...
- See [recovery-audacious.md](recovery-audacious.md) for ZFS import and chroot
  procedures

**EFI sync not working:**

- `systemctl status efi-sync.path efi-sync.service`
- `diff -r /boot/efi/EFI/Linux /boot/efi-backup/EFI/Linux` (should be
  identical)
...
- [recovery-audacious.md](recovery-audacious.md) - Boot and ZFS recovery
  procedures
- [docs/disaster-recovery.md](../disaster-recovery.md) - Disaster scenarios and
  data restore paths
- [installed-software-audacious.md](installed-software-audacious.md) - Complete
  package list
