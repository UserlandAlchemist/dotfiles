...
case "$1" in
  click)
    if probe; then
      ...
      if probe; then
        # Still up - check journalctl for the reason (get most recent message from script)
        MSG=$(ssh -o BatchMode=yes -o ConnectTimeout=2 astute \
          'sudo journalctl -u astute-idle-suspend.service -n 20 --output=cat | grep "^Astute staying awake" | tail -1' 2>/dev/null)
        if [ -n "$MSG" ]; then
